...
def complexes(pkl_dataset, output_dill, type):
    if os.path.exists(output_dill):
        log_complex.warning("Complex file {:} already exists!".format(output_dill))
        return
    log_complex.info("Getting filenames...")
    filenames = db.get_structures_filenames(pkl_dataset, extension='.pkl')
    log_complex.info("Getting complexes...")
...
def get_complexes(filenames, type):
    ...
    if type == 'rcsb':
        ...
        raw_complexes = _get_pdbbind_complexes(filenames)
    else:
...
def _get_db5_complexes(filenames, keyer=db.get_pdb_code):
    ...
    for pdb_code in pdb_codes:
        ...
        if lb is None or rb is None:
            log_complex.warning("Skipping {:} since not all bound files present."
                            .format(pdb_code))
            continue
...
def _get_pdbbind_complexes(filenames, keyer=db.get_pdb_code):
    """Get complexes for pdbbind type dataset."""
    pdb_codes = ca.get_complex_pdb_codes(filenames)
    complexes = {}
    for pdb_code in pdb_codes:
        lb = ca.find_of_type(
            pdb_code, filenames, receptor=False, bound=True, style='pdbbind')
        rb = ca.find_of_type(
            pdb_code, filenames, receptor=True, bound=True, style='pdbbind')
        pb = ca.find_of_type(
            pdb_code, filenames, receptor=True, bound=False, style='pdbbind')
        if lb is None or rb is None or rb is None:
            log_complex.warning("Skipping {:} since not all bound files present."
                            .format(pdb_code))
            continue
        """ complexes[keyer(lb)] = Complex( """
        """     name=keyer(lb), bound_filenames=[lb, rb], """
        """     unbound_filenames=[]) """
        complexes[pdb_code] = Complex(
            name=pdb_code, bound_filenames=[lb, rb, pb],
            unbound_filenames=[])
    return complexes
