...
# Modified for removing waters and ligands from proteins 
def parse_biopython_structure(pdb_filename):
    """
    Extract a Bio.PDB structure from the provided PDB file.
    ...
    if ext == ".gz":
        f = gzip.open(pdb_filename, 'rt')
    elif ".pdb" in ext:
        f = open(pdb_filename, 'r')
    else:
        raise ValueError("Unrecognized filetype " + pdb_filename)
    lines = [i for i in f.readlines() if not (i.startswith('ATOM') and i.rstrip().endswith('H') or i.startswith('HETATM') or i.startswith('HEADER') or i.startswith('TER'))]
    f.close()
    ft = tempfile.TemporaryFile(mode='w+')
    ft.writelines(lines)
    ft.seek(0)
    try:      
        biopy_structure = parser.get_structure('pdb', ft)
    except Exception:
        biopy_structure = None
    ft.close()
    return biopy_structure
