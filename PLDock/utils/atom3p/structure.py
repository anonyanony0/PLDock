...
from biopandas.mol2 import PandasMol2 as pdm
...
def parse_structure(structure_filename, concoord=False, one_model=False):
    ...
    if detailed:
        ...
        # Add ligand support
        # TODO: Find a more elegant way & pure RCSB support
        df = None
        if '.mol2' in pdb_name:
            try:
                df0 = pdm().read_mol2(structure_filename).df
                # df = pd.DataFrame([(
                #     pdb_name,
                #     '0',
                #     'A',
                ...
                #     atom.get_id()[0],
                #     atom.get_name(),
                #     str(atom.serial_number)) for atom in ],
                #     columns=[
                #         'pdb_name',
                #         'model',
                #         'chain',
                ...
                df = pd.DataFrame([(
                    ...
                    r.atom_type.split('.')[0],
                    r.atom_type,
                    str(r.atom_id)) for i, r in df0.iterrows()],
                    columns=[
                        'pdb_name',
                        'model',
                        'x',
                ...
                logger.info(f'Reading {pdb_name} error')
        else:
            atoms = []
            biopy_structure = db.parse_biopython_structure(structure_filename)
            if biopy_structure:
                for residue in \
                        Bio.PDB.Selection.unfold_entities(biopy_structure, 'R'):
                ...
                df = pd.DataFrame([(
                    ...
                    else atom.get_parent().get_full_id()[2],
...
def get_missing_sidechains(pdb_dataset, output_scwrl):
    ...
    for pdb_filename in db.get_structures_filenames(pdb_dataset):
        ...
        missing = 0
        scwrl_list = []
        log_structure.info(f"Processing {pdb_name}")
                ...
                for i, residue in enumerate(chain):
                    ...
                    if res_name not in expected:
                        log_structure.warning(
                          f"Non-standard residue found:{res_name}, Skipping.")
                        continue
                    ...
                    if curr_count != expected[res_name]:
                        log_structure.debug(
                          f'Missing residue {res_name} at position {i} with id {res_id} which has {curr_count} instead of the expected {expected[res_name]} atoms.')
                        missing += 1
                        scwrl_list.append(res_code.upper())
                    else:
                        scwrl_list.append(res_code.lower())

        log_structure.debug(f"Missing {missing} residue total")
...
def residue_to_pdbresidue(residue, r_atoms):
    ...
    for _, atom in r_atoms.iterrows():
