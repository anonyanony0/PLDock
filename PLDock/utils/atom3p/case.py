...
def generate_all_clean_complexes(args):
    ...
    log_case.info("{:} requested keys, {:} produced keys, {:} work keys"
...
def is_of_type(pdb_name, style, receptor=None, bound=None):
    ...
    if style == 'pdbbind':
        if receptor and bound:
            return _has_symbol('protein', pdb_name)
        elif not receptor and bound:
            return _has_symbol('ligand', pdb_name)
        else:
            return _has_symbol('pocket', pdb_name)
    if receptor is None:
        if bound is None:
            return True
        elif bound:
...
def _get_alignments(seq_u, atom_u, seq_b, atom_b):
    ...
    warned = False
    if len(alignments) > 1:
        log_case.warning("More than one alignment found.")
    for i, curr in enumerate(alignments):
        ...
        #        log_case.info('Alignment {:} (score {:}) from {:} to {:} on bound, '
        ...
        for k in range(align_size):
            if align_b[k] != '-' and align_u[k] != '-':
                if idx_b not in b2u and idx_u not in u2b:
                    ...
                    if not warned:
                        log_case.warning('ignoring double prediction {:} bound '
                                        'to {:} unbound'.format(idx_u, idx_b))
                        log_case.warning('not showing future warnings for this '
                                        'alignment')
                        warned = True
            if align_u[k] != '-':
                idx_u += 1
...
def _compute_similarities(unbound, bound):
    ...
    for i, chain_u in enumerate(chains_u):
        for j, chain_b in enumerate(chains_b):
            # log_case.info("unbound chain {:} vs. bound chain {:}"
...
def _get_chain_mapping(unbound, bound, style):
    ...
    for unmatched_u in [cu for cu in chains_u if cu not in u2b_chain]:
        log_case.warning("{:} chain {:} has no match found!"
            .format(
