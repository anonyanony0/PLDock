...
import sys
import json
import urllib.parse as urldict
...
def isXray( pdb ):
    ...
    expInfoURL = urllib.request.urlopen(expInfoAddress)
    ...
    """
    ###### the PDB service API have changed to json
    expInfoAddress = f'https://data.rcsb.org/rest/v1/core/entry/{pdb}'
    expInfoURL = urllib.urlopen(expInfoAddress)
    url_string = expInfoURL.read()
    url_string = url_string.decode('unicode_escape')
    url_string = json.loads(url_string,strict=False)
    #url_string = json.loads(url_string, cls=LazyDecoder)
    expMethod = url_string['exptl'][0]['method']
    ######
...
def chainPresent(pdb,chain):
    ...
    if chain in chains:
        ...
        return False
    """
    chainInfoAddress=f'https://data.rcsb.org/rest/v1/core/polymer_entity_instance/{pdb}/{chain}'
    try:
        chainInfoURL = urllib.urlopen(chainInfoAddress)
        return True
...
def fetchpdbChainsList( selectedStruture, seq_id ):
    ...
    pdbChainsList = []
    try:
        entry_id,asym_id = selectedStruture.split('.')
        chainInfoAddress=f'https://data.rcsb.org/rest/v1/core/polymer_entity_instance/{entry_id}/{asym_id}'
        chainInfoURL = urllib.urlopen(chainInfoAddress)
        url_string = chainInfoURL.read()
        url_string = url_string.decode('unicode_escape')
        #url_string = json.loads(url_string)
        url_string = json.loads(url_string, cls=LazyDecoder)
        entity_id = url_string['rcsb_polymer_entity_instance_container_identifiers']['entity_id']
        chainInfoAddress=f'https://data.rcsb.org/rest/v1/core/polymer_entity/{entry_id}/{entity_id}'
        chainInfoURL = urllib.urlopen(chainInfoAddress)
        url_string = chainInfoURL.read()
        url_string = url_string.decode('unicode_escape')
        #url_string = json.loads(url_string, strict=False)
        url_string = json.loads(url_string, cls=LazyDecoder,strict=False)
        seq = url_string['entity_poly']['pdbx_seq_one_letter_code']
        #q ={"json":{"query":{"type":"terminal","service":"sequence","parameters":{"evalue_cutoff":1,"identity_cutoff":int(seq_id)/100,"target":"pdb_protein_sequence","value":seq}},"request_options":{"scoring_strategy":"sequence"},"return_type":"polymer_instance"}}
        q ={"json":{"query":{"type":"terminal","service":"sequence","parameters":{"evalue_cutoff":1,"identity_cutoff":int(seq_id)/100,"target":"pdb_protein_sequence","value":seq}},"request_options":{"return_all_hits":True},"return_type":"polymer_instance"}}
        q = urldict.urlencode ({k: json.dumps(v) for k, v in q.items()})
        chainInfoAddress = f'https://search.rcsb.org/rcsbsearch/v2/query?{q}'
        chainInfoURL = urllib.urlopen(chainInfoAddress)
        url_string = chainInfoURL.read()
        url_string = url_string.decode('unicode_escape')
        #url_string = json.loads(url_string)
        url_string = json.loads(url_string, cls=LazyDecoder)
        #res = [i['identifier'].replace('.', ':') for i in url_string['result_set'][0] if i['score']>=seq_id]
        res = [i['identifier'].replace('.', ':') for i in url_string['result_set']]
        for i in res:
            if isXray(i.split(":")[0]):
                pdbChainsList.append(i)
        return pdbChainsList
    except URLError.HTTPError as e:
        logger.error('fetchpdbChainsList Failed')
        return []
...
def filterbyResolution( pdbChainsList, resolutionCutoff ):
    ...
    for pdb in pdbsList:
        ...
        url_string = url_string.decode('unicode_escape')
        url_string = json.loads(url_string,strict=False)
        res = url_string["pdbx_vrpt_summary"]["pdbresolution"]
