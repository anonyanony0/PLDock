...
from ..commons.utils import pmap_multi, read_strings_from_txt
...
class PDBBind(Dataset):
    ...
    def __init__(self, device='cuda:0',
                 ...
                 num_confs=10,
                 pdbbind_dir = 'data/PDBBind', #add for other data
                 bsp_dir = None, #add for other data
                 run_dir= 'data',
        ...
        self.processed_dir = f'{run_dir}/processed/size{self.dataset_size}_INDEX{os.path.splitext(os.path.basename(self.complex_names_path))[0]}_Hpolar{int(self.only_polar_hydrogens)}_H{int(not self.remove_h)}_BSPprot{int(self.bsp_proteins)}_BSPlig{int(self.bsp_ligands)}_surface{int(self.use_rec_atoms)}_pocketRad{self.pocket_cutoff}_ligRad{self.lig_graph_radius}_recRad{self.rec_graph_radius}_recMax{self.c_alpha_max_neighbors}_ligMax{self.lig_max_neighbors}_chain{self.chain_radius}_POCKET{self.pocket_mode}'
        log().info(f'using processed directory: {self.processed_dir}')
        ...
        if not os.path.exists(f'{run_dir}/processed/'):
            os.mkdir(f'{run_dir}/processed/')
        ...
        log().info('finish loading data into memory')
        self.cache = {}
    ...
    def process(self):
        log().info(f'Processing complexes from [{self.complex_names_path}] and saving it to [{self.processed_dir}]')
        ...
        log().info(f'Loading {len(complex_names)} complexes.')
        ligs = []
        to_remove = []
        for name in tqdm(complex_names, desc='loading ligands'):
            if self.bsp_ligands:
                ...
                if lig == None:
                    # ADD FOR PDB
                    if lig == None:  # read pdb file if sdf file cannot be sanitized
                        lig = read_molecule(os.path.join(self.pdbbind_dir, name, f'{name}_ligand.pdb'), sanitize=True,
                                        remove_hs=self.remove_h)
                        if lig == None:
                            lig = read_molecule(os.path.join(self.pdbbind_dir, name, f'{name}_ligand.pdb'), sanitize=False,
                                        remove_hs=self.remove_h)
                            if lig  == None:
                                to_remove.append(name)
                                print(name)
        ...
        if self.bsp_proteins:
            ...
            for name in complex_names:
                for fi in os.listdir(os.path.join(self.pdbbind_dir, name)):
                    if 'protein' in fi:
                        #rec_paths.append(os.path.join(self.pdbbind_dir, name, fi))
                        pfile = os.path.join(self.pdbbind_dir, name, fi)
                        #with warnings.catch_warnings():
                        #    warnings.filterwarnings("ignore", category=PDBConstructionWarning)
                        try:
                            structure = biopython_parser.get_structure('random_id', pfile)[0]
                            if structure:
                                rec_paths.append(pfile)
                            else:
                                to_remove.append(name)
                        except:
                            to_remove.append(name)
        ...
        if not os.path.exists(os.path.join(self.processed_dir, 'rec_graphs.pt')) or not os.path.exists(os.path.join(self.processed_dir, 'pocket_and_rec_coords.pt')) or (not os.path.exists(os.path.join(self.processed_dir, self.rec_subgraph_path)) and self.rec_subgraph):
            log().info('Get receptors, filter chains, and get its coordinates')
        ...
        if not os.path.exists(os.path.join(self.processed_dir, 'pocket_and_rec_coords.pt')):
            log().info('Get Pocket Coordinates')
            ...
            log().info('pocket_and_rec_coords.pt already exists. Using those instead of creating new ones.')

        if not os.path.exists(os.path.join(self.processed_dir, 'rec_graphs.pt')):
            log().info('Get receptor Graphs')
            ...
            log().info('rec_graphs.pt already exists. Using those instead of creating new ones.')
        log().info('Done converting to graphs')
        ...
        if self.rec_subgraph and not os.path.exists(os.path.join(self.processed_dir, self.rec_subgraph_path)):
            log().info('Get receptor subgraphs')
            ...
            log().info(os.path.join(self.processed_dir, self.rec_subgraph_path), ' already exists. Using those instead of creating new ones.')
        log().info('Done creating receptor subgraphs')

        if not os.path.exists(os.path.join(self.processed_dir, self.lig_graph_path)):
            log().info('Convert ligands to graphs')
            if self.multiple_rdkit_conformers:
            ...
            log().info('lig_graphs.pt already exists. Using those instead of creating new ones.')
        ...
        if not os.path.exists(os.path.join(self.processed_dir, 'lig_structure_graphs.pt')) and self.lig_structure_graph:
            ...
            log().info('lig_structure_graphs.pt already exists or is not needed.')

        if not os.path.exists(os.path.join(self.processed_dir, 'geometry_regularization.pt')):
            log().info('Convert ligands to geometry graph')
            ...
            log().info('geometry_regularization.pt already exists or is not needed.')
