...
from .lr_schedulers import WarmUpWrapper  # do not remove
...
from ..commons.utils import flatten_dict, tensorboard_gradient_magnitude, move_to_device, list_detach, concat_if_list
...
class Trainer():
    def __init__(self, model, args, metrics: Dict[str, Callable], main_metric: str, device: torch.device,
        ...
        log().info(f'Log directory: {self.writer.log_dir}')
    ...
    def train(self, train_loader: DataLoader, val_loader: DataLoader):
        ...
        for epoch in range(self.start_epoch, self.args.num_epochs + 1):  # loop over the dataset multiple times
            ...
            with torch.no_grad():
                ...
                log().info('[Epoch %d] %s: %.6f val loss: %.6f' % (epoch, self.main_metric, val_score, val_loss))
                ...
                log().info('Epochs with no improvement: [', epochs_no_improve, '] and the best  ', self.main_metric,
                ...
                if epochs_no_improve >= self.args.patience and epoch >= self.args.minimum_epochs:  # stopping criterion
                    log().info(f'Early stopping criterion based on -{self.main_metric}- that should be {self.main_metric_goal}-imized reached after {epoch} epochs. Best model checkpoint was in epoch {epoch - epochs_no_improve}.')
                    break
            ...
            *batch, batch_indices = move_to_device(list(batch), self.device)
            ...
            with torch.no_grad():
                ...
                if self.optim_steps % self.args.log_iterations == 0 and optim != None:
                    ...
                    log().info('[Epoch %d; Iter %5d/%5d] %s: loss: %.7f' % (
    ...
    def evaluation(self, data_loader: DataLoader, data_split: str = '', return_pred=False):
        ...
        with open(os.path.join(self.writer.log_dir, 'evaluation_' + data_split + '.txt'), 'w') as file:
            log().info('Statistics on ', data_split)
            for key, value in metrics.items():
