...
def get_receptor_atom_subgraph(rec, rec_coords, lig,  lig_coords=None ,graph_cutoff=4, max_neighbor=8, subgraph_radius=7):
    ...
    for i in range(len(subgraph_coords)):
        ...
        if len(dst) == 0:
            ...
            log().info(f'The graph_cutoff {graph_cutoff} was too small for one c_alpha such that it had no neighbors. So we connected it to the closest other c_alpha')
        assert i not in dst
        src = [i] * len(dst)
        src_list.extend(src)
        dst_list.extend(dst)
    ...
    log().info('number of subgraph nodes = ', len(subgraph_coords), ' number of edges in subgraph = ', len(dist_list) )
    return graph
...
def get_pocket_coords(lig, rec_coords, cutoff=5.0, pocket_mode='match_atoms'):
    ...
    log().info('Num pocket nodes = ', len(pocket_coords), 'ligand nodes = ', lig_coords.shape[0],
...
def get_lig_graph_multiple_conformer(mol, name, radius=20, max_neighbors=None, use_rdkit_coords=False, num_confs=10):
    ...
    for i in range(num_confs):
        ...
        log().info('kabsch RMSD between rdkit ligand and true ligand is ',
...
def get_lig_graph_revised(mol, name, radius=20, max_neighbors=None, use_rdkit_coords=False):
    ...
    if use_rdkit_coords:
        try:
            ...
            log().info('kabsch RMSD between rdkit ligand and true ligand is ', np.sqrt(np.sum((lig_coords - true_lig_coords) ** 2, axis=1).mean()).item())
        except Exception as e:
