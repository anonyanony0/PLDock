...
def parse_arguments():
    ...
    p.add_argument('--config', type=argparse.FileType(mode='r'), default=os.path.join(package_path,'models','Equibind','configs_clean','inference_file_for_reproduce.yml'))
    ...
    p.add_argument('--run_dirs', type=list, default=[os.path.join(package_path,'models','Equibind','runs','flexible_self_docking')], help='path directory with saved runs')
    ...
    p.add_argument('--test_names', type=str, default=None, help='path of test list')
    return p.parse_args()
...
def inference(args, tune_args=None):
    ...
    sys.stdout = Logger(logpath=os.path.join(args.output_directory, f'inference.log'), syspart=sys.stdout)
    sys.stderr = Logger(logpath=os.path.join(args.output_directory, f'inference.log'), syspart=sys.stderr)
    seed_all(args.seed)
    ...
    data = PDBBind(device=device, complex_names_path=args.test_names,pdbbind_dir=args.inference_path, run_dir = args.output_directory, **args.dataset_params)
            ...
            for i, (prediction, target, lig_keypts, rec_keypts, name) in tqdm(enumerate(
                ...
                if lig == None:
                    ...
                    if lig == None:  # read pdb file if sdf file cannot be sanitized
                        lig = read_molecule(os.path.join(self.inference_path, name, f'{name}_ligand.pdb'), sanitize=True)
                        if lig == None:
                            lig = read_molecule(os.path.join(self.inference_path, name, f'{name}_ligand.pdb'), sanitize=False)
                            if lig  == None:
                                print(name)
...
def inference_from_files(args):
    ...
    names = os.listdir(args.inference_path) if not args.test_names  else tqdm(read_strings_from_txt(args.test_names))
    for idx, name in enumerate(names):
        #if not os.path.exists(f'{args.output_directory}/{name}/lig_equibind_corrected.sdf'):
        ...
        for lig_name in lig_names:
            ...
            #check geometry_regularization
            if ('geometry_regularization' in dp and dp['geometry_regularization']) or ('geometry_regularization_ring' in dp and dp['geometry_regularization_ring']):
                try: 
                    tmplig = deepcopy(lig)
                    lig_graph = get_lig_graph_revised(tmplig, name, max_neighbors=dp['lig_max_neighbors'],
                                          use_rdkit_coords=use_rdkit_coords, radius=dp['lig_graph_radius'])
                    geometry_graph = get_geometry_graph(tmplig)
                except:
                    lig = None
            if lig != None:  # read mol2 file if sdf file cannot be sanitized
        ...
        if lig == None: 
            ...
            print(f'None of the ligand files could be read: {lig_names}')
        else:
            ...
            try:
                ...
                print(f'Something Error in {name}')

    #path = os.path.join(os.path.dirname(args.checkpoint), f'predictions_RDKit{use_rdkit_coords}.pt')
    path = os.path.join(args.output_directory, f'predictions_RDKit{use_rdkit_coords}.pt')
    print(f'Saving predictions to {path}')
...
def equibind_test(**kwargs):
    ...
    for run_dir in args.run_dirs:
        ...
        args.checkpoint = f'{run_dir}/best_checkpoint.pt'
        config_dict['checkpoint'] = f'{run_dir}/best_checkpoint.pt'
        ...
        for key, value in checkpoint_dict.items():
            if key not in config_dict.keys():
                if isinstance(value, list):
                    ...
                    arg_dict[key] = value
        arg_dict['test_names']=None
        if 'config' in kwargs:
            arg_dict = args.__dict__
            with open(kwargs['config'], 'r') as arg_file:
                config_dict = yaml.load(arg_file, Loader=yaml.FullLoader)
                for key, value in config_dict.items():
                    if isinstance(value, list):
                        for v in value:
                            arg_dict[key].append(v)
