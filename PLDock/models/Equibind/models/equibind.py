...
class IEGMN_Layer(nn.Module):
    ...
    def forward(self, lig_graph, rec_graph, coords_lig, h_feats_lig, original_ligand_node_features, orig_coords_lig,
        ...
        with lig_graph.local_scope() and rec_graph.local_scope():
            ...
            if self.debug:
                log().info(torch.max(lig_graph.ndata['x_now'].abs()), 'x_now : x_i at layer entrance')
                log().info(torch.max(lig_graph.ndata['feat'].abs()), 'data[feat] = h_i at layer entrance')

            if self.lig_evolve:
                lig_graph.apply_edges(fn.u_sub_v('x_now', 'x_now', 'x_rel'))  # x_i - x_j
                if self.debug:
                    log().info(torch.max(lig_graph.edata['x_rel'].abs()), 'x_rel : x_i - x_j')
            if self.rec_evolve:
            ...
            if self.debug:
                log().info(torch.max(lig_graph.edata['msg'].abs()),
            ...
            if self.debug:
                log().info(torch.max(cross_attention_lig_feat.abs()), 'aggr_cross_msg(i) = sum_j a_{i,j} * h_j')
            ...
            if self.debug:
                log().info(torch.max(lig_graph.ndata['aggr_msg'].abs()), 'data[aggr_msg]: \sum_j m_{i->j} ')
                if self.lig_evolve:
                    log().info(torch.max(lig_graph.ndata['x_update'].abs()),
                        'data[x_update] : \sum_j (x_i - x_j) * \phi^x(m_{i->j})')
                    log().info(torch.max(x_evolved_lig.abs()), 'x_i new = x_evolved_lig : x_i + data[x_update]')
            ...
            if self.debug:
                log().info('node_mlp params')
                for p in self.node_mlp.parameters():
                    log().info(torch.max(p.abs()), 'max node_mlp_params')
                    log().info(torch.min(p.abs()), 'min of abs node_mlp_params')
                log().info(torch.max(input_node_upd_ligand.abs()), 'concat(h_i, aggr_msg, aggr_cross_msg)')
                log().info(torch.max(node_upd_ligand), 'h_i new = h_i + MLP(h_i, aggr_msg, aggr_cross_msg)')
...
class IEGMN(nn.Module):
    ...
    def forward(self, lig_graph, rec_graph, geometry_graph, complex_names, epoch):
        ...
        if self.debug:
            log().info(torch.max(h_feats_lig.abs()), 'max h_feats_lig before layers and noise ')
            log().info(torch.max(h_feats_rec.abs()), 'max h_feats_rec before layers and noise ')
        ...
        if self.debug:
            log().info(torch.max(h_feats_lig.abs()), 'h_feats_lig before layers but after noise ')
            log().info(torch.max(h_feats_rec.abs()), 'h_feats_rec before layers but after noise ')
        ...
        if self.debug:
            log().info(torch.max(h_feats_lig.abs()), torch.norm(h_feats_lig),
                'max and norm of h_feats_lig before layers but after noise and mu_r_norm')
            log().info(torch.max(h_feats_rec.abs()), torch.norm(h_feats_lig),
        ...
        for i, layer in enumerate(self.iegmn_layers):
            if self.debug: log().info('layer ', i)
            coords_lig, \
            h_feats_lig, \
            coords_rec, \
        ...
        if self.debug:
            log().info(torch.max(h_feats_lig.abs()), 'max h_feats_lig after MPNN')
            log().info(torch.max(coords_lig.abs()), 'max coords_lig before after MPNN')
        ...
        for idx in range(len(ligs_node_idx) - 1):
            ...
            if torch.isnan(lig_keypts).any():
                log().info(complex_names, 'complex_names where Nan encountered')
            assert not torch.isnan(lig_keypts).any()
            if torch.isinf(lig_keypts).any():
                log().info(complex_names, 'complex_names where inf encountered')
            ...
            while torch.min(S) < 1e-3 or torch.min(
                ...
                if self.debug: log().info('S inside loop ', num_it, ' is ', S, ' and A = ', A)
            ...
            if self.debug:
                log().info('rec_keypts_mean', rec_keypts_mean)
                log().info('lig_keypts_mean', lig_keypts_mean)
...
class EquiBind(nn.Module):
    ...
    def forward(self, lig_graph, rec_graph, geometry_graph=None, complex_names=None, epoch=0):
        if self.debug: log().info(complex_names)
        ...
        for idx in range(len(ligs_node_idx) - 1):
            ...
            if self.debug:
                ...
                log().info('rotation @ rotation.t() - eye(3)', rotation @ rotation.t() - torch.eye(3).to(self.device))
