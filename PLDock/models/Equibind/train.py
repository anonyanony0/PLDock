...
from PLDock import log,package_path
...
def parse_arguments():
    ...
    p.add_argument('--config', type=argparse.FileType(mode='r'), default=os.path.join(package_path,'models','Equibind','configs_clean','RDKitCoords_flexible_self_docking.yml'))
    ...
    p.add_argument('--pdb_dir', type=str, default='data/pdbbind', help='pdb directory')
    return p.parse_args()
...
def train(args, run_dir):
    ...
    train_data = PDBBind(device=device, complex_names_path=args.train_names,lig_predictions_name=args.train_predictions_name, is_train_data=True,pdbbind_dir=args.pdb_dir,run_dir = args.logdir,**args.dataset_params)
    val_data = PDBBind(device=device, complex_names_path=args.val_names,lig_predictions_name=args.val_predictions_name,pdbbind_dir=args.pdb_dir,run_dir = args.logdir,**args.dataset_params)
    ...
    log().info('train size: ', len(train_data))
    log().info('val size: ', len(val_data))

    model = load_model(args, data_sample=train_data[0], device=device)
    log().info('trainable params in model: ', sum(p.numel() for p in model.parameters() if p.requires_grad))
    ...
    if args.eval_on_test:
        ...
        test_data = PDBBind(device=device, complex_names_path=args.test_names,pdbbind_dir=args.pdb_dir ,run_dir = args.logdir,**args.dataset_params)
        test_loader = DataLoader(test_data, batch_size=args.batch_size, collate_fn=collate_function,
                                 pin_memory=args.pin_memory, num_workers=args.num_workers)
        log().info('test size: ', len(test_data))
    ...
    if args.multithreaded_seeds != []:
        with concurrent.futures.ThreadPoolExecutor() as executor:
            ...
            for seed in args.multithreaded_seeds:
                args_copy = get_arguments(**kwargs)
                args_copy.seed = seed
